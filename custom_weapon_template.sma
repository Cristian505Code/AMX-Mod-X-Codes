...
// ~ [ Supported Modes ] ~ //
//#define ZOMBIE_PLAGUE_SUPPORT // Remove "//" if you want to have zp support
#define CLASSIC_SUPPORT

#if defined ZOMBIE_PLAGUE_SUPPORT
#include <zombieplague>

new const ITEM_NAME[] = "x";
const ITEM_AMMOPACKS_PRICE = x;
const ITEM_TEAM = ZP_TEAM_HUMAN;
#endif
...
const Float: WEAPON_SHOOT_RATE = x;
const Float: WEAPON_SHOOT_PUNCHANGLE = x;
const Float: WEAPON_SHOOT_DAMAGE = x;
...
// ~ [ Anims Settings ] ~ //
const Float: WEAPON_ANIM_IDLE_TIME = x;
const Float: WEAPON_ANIM_RELOAD_TIME = x;
const Float: WEAPON_ANIM_DRAW_TIME = x;
const Float: WEAPON_ANIM_SHOOT_TIME = x;
...
public plugin_init()
{
    register_plugin("Custom Weapon Template", "2.0", "Cristian505 \ Batcoh: Code Base");

    #if defined ZOMBIE_PLAGUE_SUPPORT 
    g_ItemIndex = zp_register_extra_item(ITEM_NAME, ITEM_AMMOPACKS_PRICE, ITEM_TEAM);
    #endif
...
#if defined ZOMBIE_PLAGUE_SUPPORT
public zp_extra_item_selected(iPlayer, iItem) if(iItem == g_ItemIndex) Command_GiveWeapon(iPlayer);
#endif
